...
- Get the involved commits between the tag and the previous one (could not be the same package, tre previous is sense of time)
- Extracts the task number from the merged Pull Requests, (commit must follow `commitlint` format `XX-000`)
...
npm run local-test
 ```
to test the action locally with the package [Local Action Debugger](https://github.com/github/local-action). Set the variables in 
```
.local.action.env
```
in particular:
